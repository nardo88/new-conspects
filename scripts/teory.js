const teory = {

    html: [{
            title: 'Теги для работы с текстом',
            path: './conspects/teory/html/teg_for_text/tegfortext.html',
        },
        {
            title: 'Ссылки',
            path: './conspects/teory/html/links/links.html',
        },
        {
            title: 'Списки',
            path: './conspects/teory/html/lists/lists.html',
        },
        {
            title: 'Таблицы',
            path: './conspects/teory/html/table/table.html',
        },
        {
            title: 'Input',
            path: './conspects/teory/html/input/input.html',
        },
        {
            title: 'Textarea',
            path: './conspects/teory/html/textarea/textarea.html',
        },
        {
            title: 'Audio',
            path: './conspects/teory/html/audio/audio.html',
        },
        {
            title: 'Video',
            path: './conspects/teory/html/video/video.html',
        },

    ],
    // ===================================================================================== CSS ================================
    css: [{
            title: 'Background',
            path: './conspects/teory/css/background/background.html',
        },
        {
            title: 'Основные свойства',
            path: './conspects/teory/css/property/property.html',
        },
        {
            title: 'Тени',
            path: './conspects/teory/css/box-shadow/box-shadow.html',
        },
        {
            title: 'Анимация',
            path: './conspects/teory/css/animation/animation.html',
        },
        {
            title: 'Фильтры',
            path: './conspects/teory/css/filters/filters.html',
        },
        {
            title: 'Grid',
            path: './conspects/teory/css/grid/grid.html',
        },
        {
            title: 'Flex',
            path: './conspects/teory/css/flex/flex.html',
        },
        {
            title: 'Установка двойного фона',
            path: './conspects/teory/css/dvoynoy_fon/dvoynoy_fon.html',
        },
        {
            title: 'Как нарисовать треугольник',
            path: './conspects/teory/css/treugolnik/index.html',
        },


    ],

    // ================================================================================ PHP =============================================
    php: [{
            title: 'Переменные и операторы',
            path: './conspects/teory/php/variables_and_operators/index.html',
        },
        {
            title: 'Типы данных',
            path: './conspects/teory/php/type_of_data/index.html',
        },
        {
            title: 'Методы массивов',
            path: './conspects/teory/php/array_method/index.html',
        },
        {
            title: 'Условные конструкции',
            path: './conspects/teory/php/if_else/index.html',
        },
        {
            title: 'Циклы',
            path: './conspects/teory/php/cycles/index.html',
        },
        {
            title: 'Функции',
            path: './conspects/teory/php/function/index.html',
        },
        {
            title: 'Goto (метки)',
            path: './conspects/teory/php/goto/index.html',
        },
        {
            title: 'GET и POST',
            path: './conspects/teory/php/get_post/index.html',
        },
        {
            title: 'Классы',
            path: './conspects/teory/php/class/index.html',
        },
        {
            title: 'Работа с датами',
            path: './conspects/teory/php/time/index.html',
        },
        {
            title: 'Базы данных',
            path: './conspects/teory/php/data_base/index.html',
        },
        {
            title: 'Работа с БД средствами php',
            path: './conspects/teory/php/use_database_with_php/index.html',
        },
        {
            title: 'Cookies',
            path: './conspects/teory/php/cookies/index.html',
        },
        {
            title: 'Session',
            path: './conspects/teory/php/session/index.html',
        },
        {
            title: 'Основы безопасности',
            path: './conspects/teory/php/anti_hack/index.html',
        },
        {
            title: 'Работа с файлами',
            path: './conspects/teory/php/files/index.html',
        },

    ],


    // ================================================================================ JavaScript =============================================
    JavaScript: [{
            title: 'Типы данных',
            path: './conspects/teory/javaScript/type_data/index.html',
        },
        {
            title: 'Работа со строками',
            path: './conspects/teory/javaScript/string/index.html',
        },
        {
            title: 'Импорт библиотек в JS',
            path: './conspects/teory/javaScript/import/index.html',
        },
        {
            title: 'Модули',
            path: './conspects/teory/javaScript/modul/index.html',
        },
        {
            title: 'Математические операторы',
            path: './conspects/teory/javaScript/math/index.html',
        },
        {
            title: 'Условные операторы',
            path: './conspects/teory/javaScript/uslovnie_operatory/index.html',
        },
        {
            title: 'Циклы',
            path: './conspects/teory/javaScript/cikli/index.html',
        },
        {
            title: 'События',
            path: './conspects/teory/javaScript/sobitiya/index.html',
        },
        {
            title: 'Объект события',
            path: './conspects/teory/javaScript/event/index.html',
        },
        {
            title: 'Массивы',
            path: './conspects/teory/javaScript/array/index.html',
        },
        {
            title: 'Объекты',
            path: './conspects/teory/javaScript/object/index.html',
        },
        {
            title: 'Классы',
            path: './conspects/teory/javaScript/class/index.html',
        },
        {
            title: 'Регулярные выражения',
            path: './conspects/teory/javaScript/regular_expression/index.html',
        },
        {
            title: 'Функции',
            path: './conspects/teory/javaScript/functions/index.html',
        },
        {
            title: 'Работа с DOM',
            path: './conspects/teory/javaScript/dom/index.html',
        },
        {
            title: 'setInterval',
            path: './conspects/teory/javaScript/setInterval/index.html',
        },
        {
            title: 'requestAnimationFrame',
            path: './conspects/teory/javaScript/requestAnimationFrame/index.html',
        },
        {
            title: 'Storage',
            path: './conspects/teory/javaScript/storage/index.html',
        },
        {
            title: 'JSON',
            path: './conspects/teory/javaScript/json/index.html',
        },
        {
            title: 'Прототипы',
            path: './conspects/teory/javaScript/prototype/index.html',
        },
        {
            title: 'Передача контекста',
            path: './conspects/teory/javaScript/call_bind_aply/index.html',
        },
        {
            title: 'Замыкание',
            path: './conspects/teory/javaScript/closure/index.html',
        },
        {
            title: 'Асинхронность',
            path: './conspects/teory/javaScript/async/index.html',
        },
        {
            title: 'Промисы',
            path: './conspects/teory/javaScript/promise/index.html',
        },
        {
            title: 'Async, Await',
            path: './conspects/teory/javaScript/async_await/index.html',
        },
        {
            title: 'Map, Set',
            path: './conspects/teory/javaScript/map_set/index.html',
        },
        {
            title: 'XHR, Fetch',
            path: './conspects/teory/javaScript/xhr_fetch/index.html',
        },
        {
            title: 'Spread, Rest',
            path: './conspects/teory/javaScript/spread_rest/index.html',
        },
        {
            title: 'Диструктуризация',
            path: './conspects/teory/javaScript/distructurization/index.html',
        },
        {
            title: 'Drag and Drop',
            path: './conspects/teory/javaScript/drag_and_drop/index.html',
        },
        {
            title: 'Canvas',
            path: './conspects/teory/javaScript/canvas/index.html',
        },
        {
            title: 'Делегирование',
            path: './conspects/teory/javaScript/delegation/index.html',
        },
        {
            title: 'Debounce и Throttling',
            path: './conspects/teory/javaScript/Throttling/index.html',
        },
        {
            title: 'Мемоизация',
            path: './conspects/teory/javaScript/memo/index.html',
        },
        {
            title: 'Тестирование',
            path: './conspects/teory/javaScript/testing/index.html',
        },
        {
            title: 'Полифилы',
            path: './conspects/teory/javaScript/polyfills/index.html',
        },
        {
            title: 'Date',
            path: './conspects/teory/javaScript/date/index.html',
        },

    ],



    // ================================================================================ React =============================================
    React: [
        {
            title: 'Подключение React через CDN',
            path: './conspects/teory/react/add_from_cdn/index.html',
        },
        {
            title: 'create-react-app',
            path: './conspects/teory/react/create_react_app/index.html',
        },
        {
            title: 'Особенности JSX',
            path: './conspects/teory/react/jsx/index.html',
        },
        {
            title: 'Работа со стилями в React',
            path: './conspects/teory/react/style/index.html',
        },
        {
            title: 'Компонента',
            path: './conspects/teory/react/componenta/index.html',
        },
        {
            title: 'Props',
            path: './conspects/teory/react/props/index.html',
        },
        {
            title: 'Роутинг',
            path: './conspects/teory/react/route/index.html',
        },
        {
            title: 'Мапинг',
            path: './conspects/teory/react/map/index.html',
        },
        {
            title: 'События',
            path: './conspects/teory/react/events/index.html',
        },
        {
            title: 'Ref',
            path: './conspects/teory/react/ref/index.html',
        },
        {
            title: 'Сallback через props',
            path: './conspects/teory/react/callback_props/index.html',
        },
        {
            title: 'Flux',
            path: './conspects/teory/react/flux/index.html',
        },
        {
            title: 'Циклическая зависимость',
            path: './conspects/teory/react/cyclical_dependence/index.html',
        },
        {
            title: 'ООП в React',
            path: './conspects/teory/react/oop/index.html',
        },
        {
            title: 'Dispatch',
            path: './conspects/teory/react/dispatch/index.html',
        },
        {
            title: 'Reducer',
            path: './conspects/teory/react/reduser/index.html',
        },
        {
            title: 'Redux',
            path: './conspects/teory/react/redux/index.html',
        },
        {
            title: 'Контейнерные компоненты',
            path: './conspects/teory/react/container/index.html',
        },
        {
            title: 'Context API',
            path: './conspects/teory/react/context/index.html',
        },
        {
            title: 'React-Redux',
            path: './conspects/teory/react/react-redux/index.html',
        },
        {
            title: 'Rest API',
            path: './conspects/teory/react/rest-api/index.html',
        },
        {
            title: 'Классовые компоненты',
            path: './conspects/teory/react/class/index.html',
        },
        {
            title: 'Жизненный цикл компоненты',
            path: './conspects/teory/react/lifecycle/index.html',
        },
        {
            title: 'Пагинация',
            path: './conspects/teory/react/pagination/index.html',
        },
        {
            title: 'Хок withRouter',
            path: './conspects/teory/react/withRouter/index.html',
        },
        {
            title: 'Логинизация',
            path: './conspects/teory/react/login/index.html',
        },
        {
            title: 'DAL (уровень доступа к данным)',
            path: './conspects/teory/react/dal/index.html',
        },
        {
            title: 'Redux-Thunk',
            path: './conspects/teory/react/thunk/index.html',
        },
        {
            title: 'Redirect',
            path: './conspects/teory/react/redirect/index.html',
        },
        {
            title: 'High Order Component (ХОК)',
            path: './conspects/teory/react/hoc/index.html',
        },
        {
            title: 'Compose',
            path: './conspects/teory/react/compose/index.html',
        },
        {
            title: 'Local state',
            path: './conspects/teory/react/local_state/index.html',
        },
        {
            title: 'Redux-form',
            path: './conspects/teory/react/redux_form/index.html',
        },
        {
            title: 'LogIn LogOut',
            path: './conspects/teory/react/login_logout/index.html',
        },
        {
            title: 'Инициализация приложения',
            path: './conspects/teory/react/initial/index.html',
        },
        {
            title: 'Reselect',
            path: './conspects/teory/react/reselect/index.html',
        },
        {
            title: 'Оптимизация',
            path: './conspects/teory/react/optimization/index.html',
        },
        {
            title: 'Тестирование',
            path: './conspects/teory/react/testing/index.html',
        },
        {
            title: 'Загрузка файлов на сервер',
            path: './conspects/teory/react/upload_files/index.html',
        },
        {
            title: 'Deploy на GitHub',
            path: './conspects/teory/react/deploy/index.html',
        },
        {
            title: 'useState',
            path: './conspects/teory/react/useState/index.html',
        },
        {
            title: 'useEffect',
            path: './conspects/teory/react/useEffect/index.html',
        },
        {
            title: 'useRef',
            path: './conspects/teory/react/useRef/index.html',
        },
        {
            title: 'useMemo',
            path: './conspects/teory/react/useMemo/index.html',
        },
        {
            title: 'useContext',
            path: './conspects/teory/react/useContext/index.html',
        },
        {
            title: 'useReducer',
            path: './conspects/teory/react/useReducer/index.html',
        },
        {
            title: 'propTypes',
            path: './conspects/teory/react/propTypes/index.html',
        },



    ],



    // ================================================================================ TypeScript =============================================
    TypeScript: [
        {
            title: 'Инсталяция',
            path: './conspects/teory/TypeScript/install/index.html',
        },
        {
            title: 'Компилятор',
            path: './conspects/teory/TypeScript/compiler/index.html',
        },
        {
            title: 'Типы данных',
            path: './conspects/teory/TypeScript/types/index.html',
        },
        {
            title: 'Собственные типы данных',
            path: './conspects/teory/TypeScript/ownTypes/index.html',
        },
        {
            title: 'Интерфейсы',
            path: './conspects/teory/TypeScript/interfaces/index.html',
        },
        {
            title: 'Enums',
            path: './conspects/teory/TypeScript/Enums/index.html',
        },
        {
            title: 'Функции',
            path: './conspects/teory/TypeScript/functions/index.html',
        },
        {
            title: 'Классы',
            path: './conspects/teory/TypeScript/class/index.html',
        },
        {
            title: 'Guards',
            path: './conspects/teory/TypeScript/Guards/index.html',
        },
        {
            title: 'Generics',
            path: './conspects/teory/TypeScript/Generics/index.html',
        },
        {
            title: 'Create ReactAPP with TS',
            path: './conspects/teory/TypeScript/Create_ReactAPP_with_TS/index.html',
        },
        {
            title: 'Reducer with TS',
            path: './conspects/teory/TypeScript/reducer_ts/index.html',
        },
        {
            title: 'Типизация функционального компонента',
            path: './conspects/teory/TypeScript/function_component/index.html',
        },
        {
            title: 'Типизация классового компонента',
            path: './conspects/teory/TypeScript/class_component/index.html',
        },
        {
            title: 'Типизация state из Redux',
            path: './conspects/teory/TypeScript/stateType/index.html',
        },
        {
            title: 'Типизация axios (DLL)',
            path: './conspects/teory/TypeScript/axios/index.html',
        },
        
        


],



    // ================================================================================ Wordpress =============================================


    Wordpress: [{
            title: 'Установка на OpenServer',
            path: './conspects/teory/Wordpress/ustanovka/index.html',
        },
        {
            title: 'Deploy на хостинг',
            path: './conspects/teory/Wordpress/deploy/index.html',
        },
         {
            title: 'Создание собственной темы',
            path: './conspects/teory/Wordpress/own_theme/index.html',
        },
        {
            title: 'Functions.php',
            path: './conspects/teory/Wordpress/functions/index.html',
        },
        {
            title: 'Главная страница',
            path: './conspects/teory/Wordpress/rewrite_main_page/index.html',
        },
        {
            title: 'BlogInfo',
            path: './conspects/teory/Wordpress/blogInfo/index.html',
        },
        {
            title: 'Регистрация меню',
            path: './conspects/teory/Wordpress/reg_menu/index.html',
        },
        {
            title: 'Сайдбар',
            path: './conspects/teory/Wordpress/sidebar/index.html',
        },
        {
            title: 'Вывод записей',
            path: './conspects/teory/Wordpress/vivod_zapisey/index.html',
        },
        {
            title: 'Шаблон для записей/страниц',
            path: './conspects/teory/Wordpress/shablon/index.html',
        },
        {
            title: 'Свои типы записей',
            path: './conspects/teory/Wordpress/svoi_tipi_zapisey/index.html',
        },
        {
            title: 'Произвольные поля',
            path: './conspects/teory/Wordpress/proizvolnie_polya/index.html',
        },
        {
            title: 'Формы обратной связи',
            path: './conspects/teory/Wordpress/forma_obratnoy_svyazi/index.html',
        },
        {
            title: 'Выводим посты в слайдере',
            path: './conspects/teory/Wordpress/posti_v_slaydere/index.html',
        },

    ],
    FireBase: [
        {
            title: 'Инициализация и deploy react приложения на хостинг FireBase',
            path: './conspects/teory/Firebase/init/index.html',
        },
        {
            title: 'Авторизаяция в React приложении средствами FireBase',
            path: './conspects/teory/Firebase/authorizate/index.html',
        },
        {
            title: 'RealTime DataBase',
            path: './conspects/teory/Firebase/dataBase/index.html',
        },
        {
            title: 'Получение данных из Realtime Database',
            path: './conspects/teory/Firebase/getDataFromBase/index.html',
        },

    ],
    Vue: [{
            title: 'Инсталяция фреймворка',
            path: './conspects/teory/Vue/install/index.html',
        },

    ],
    Woocommerce: [
        {
            title: 'Установка Woocommerce',
            path: './conspects/teory/woocommerce/install/index.html',
        },
        {
            title: 'Создание темы',
            path: './conspects/teory/woocommerce/create-theme/index.html',
        },
        {
            title: 'Подключаем стили и скрипты',
            path: './conspects/teory/woocommerce/add_styles_and_scripts/index.html',
        },
        {
            title: 'Форма регистрации и авторизации',
            path: './conspects/teory/woocommerce/auth/index.html',
        },
        {
            title: 'Форма поиска на сайте',
            path: './conspects/teory/woocommerce/search_form/index.html',
        },
        {
            title: 'Корзина',
            path: './conspects/teory/woocommerce/cart/index.html',
        },


    ],
    Meteor: [
        {
            title: 'Создание нового приложения',
            path: './conspects/teory/meteor/create_app/index.html',
        },
        {
            title: 'Создание нового компонента',
            path: './conspects/teory/meteor/create_component/index.html',
        },
        {
            title: 'Collections',
            path: './conspects/teory/meteor/collections/index.html',
        },
        {
            title: 'Формы и события',
            path: './conspects/teory/meteor/forms_and_events/index.html',
        },
        {
            title: 'Создание пользователей',
            path: './conspects/teory/meteor/users/index.html',
        },

    ],

    MongoDB: [
        {
            title: 'Установка среды',
            path: './conspects/teory/mongoDb/install/index.html',
        },
        {
            title: 'Работа с БД',
            path: './conspects/teory/mongoDb/create/index.html',
        },
        {
            title: 'Atlas',
            path: './conspects/teory/mongoDb/atlas/index.html',
        },
        {
            title: 'Агрегация',
            path: './conspects/teory/mongoDb/agregatin/index.html',
        },
     

    ],

    NextJS: [
        {
            title: 'Установка среды',
            path: './conspects/teory/nextjs/install/index.html',
        },
        {
            title: 'Routing',
            path: './conspects/teory/nextjs/routing/index.html',
        },
        {
            title: 'Head',
            path: './conspects/teory/nextjs/head/index.html',
        },
        {
            title: 'Layout',
            path: './conspects/teory/nextjs/layout/index.html',
        },
        {
            title: 'Document',
            path: './conspects/teory/nextjs/document/index.html',
        },
        {
            title: 'Стили',
            path: './conspects/teory/nextjs/styles/index.html',
        },
        {
            title: 'страница 404',
            path: './conspects/teory/nextjs/404/index.html',
        },
        {
            title: 'getInitialProps (устар.)',
            path: './conspects/teory/nextjs/getInitialProps/index.html',
        },
        {
            title: 'getStaticProps',
            path: './conspects/teory/nextjs/getStaticProps/index.html',
        },
        {
            title: 'getServerSideProps',
            path: './conspects/teory/nextjs/getServerSideProps/index.html',
        },
        {
            title: 'MongoDB',
            path: './conspects/teory/nextjs/mongodb/index.html',
        },
    ],
    NodeJS: [
        {
            title: 'Модули',
            path: './conspects/teory/nodejs/modules/index.html',
        },
        {
            title: 'Взаимодействие с ОС',
            path: './conspects/teory/nodejs/os/index.html',
        },
        {
            title: 'Создание Rest API',
            path: './conspects/teory/nodejs/restApi/index.html',
        },
        {
            title: 'Подключение к БД',
            path: './conspects/teory/nodejs/connectToDb/index.html',
        },
        {
            title: 'Запись данных в БД',
            path: './conspects/teory/nodejs/addDataToBd/index.html',
        },
        {
            title: 'Маршрутизация',
            path: './conspects/teory/nodejs/router/index.html',
        },
        {
            title: 'Controllers',
            path: './conspects/teory/nodejs/controllers/index.html',
        },
        {
            title: 'Pagination',
            path: './conspects/teory/nodejs/pagination/index.html',
        },
        {
            title: 'Real Time Client-Server (long-pulling)',
            path: './conspects/teory/nodejs/longPulling/index.html',
        },
        {
            title: 'Real Time Client-Server (event soursing)',
            path: './conspects/teory/nodejs/eventSoursing/index.html',
        },
        {
            title: 'Real Time Client-Server (WebSocket)',
            path: './conspects/teory/nodejs/WebSocket/index.html',
        },
       
       
    ],
    


}

export default teory;